...
export const IGNORE_BY_DEFAULT: readonly RegExp[] = [
  ...
  /\b\d{4}-\d{2}-\d{2}\b/g,
  // Relative clocks (posted-N-ago before the bare "N hours ago" token)
  ...
  // Cookie banners
  /\b(?:we use cookies|this site uses cookies|cookie (?:banner|consent|policy|settings)|accept (?:all )?cookies|manage (?:all )?cookies|reject (?:all )?cookies)\b/gi,
];
