import { classifyOrderPlaced } from "./order-placed.js";
import {
  orderPlacedBenchCases,
  type BenchExpect,
  type OrderPlacedBenchCase,
} from "./order-placed-bench-cases.js";
...
function pageFromCase(c: OrderPlacedBenchCase): FetchedPage {
  const html = c.html ?? "";
...
export function runOrderPlacedCase(c: OrderPlacedBenchCase): ConfirmVerdictStatus {
  const page = pageFromCase(c);
  return classifyOrderPlaced(page, {
    cookiesUsed: c.cookiesUsed,
    claim: c.claim,
  }).verdict;
}

export function runOrderPlacedBench(
  cases: OrderPlacedBenchCase[] = orderPlacedBenchCases(),
): BenchReport {
  ...
  for (const c of cases) {
    const got = runOrderPlacedCase(c);
...
export function formatBenchReport(report: BenchReport): string {
  const lines = [
    "order_placed honesty bench",
    `N=${report.n}`,
    `true_placed=${report.true_placed} true_failed=${report.true_failed} trap=${report.trap}`,
