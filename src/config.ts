...
/** Health / OpenAPI — Sentinel watch renew (same $2.50 product as create). */
export const WATCH_RENEW_DESCRIPTION =
  "Livecheck Sentinel watch renew — extend an active 30-day standard watcher. POST /v1/watch/renew with {id}. Requires X-Livecheck-Owner-Token plus x402 payment. Same $2.50 USDC as POST /v1/watch create (one accept). Adds one prepaid term to expires_at (from the later of now and current expiry) and one term of checks_remaining. Same watcher id, detectors, callback, and chain balance — not a second product. Stopped or expired watchers cannot renew. owner_token is not returned again. Signed wrn_ receipt. Not /v1/watch/fast and not Confirm.";
...
export const WATCH_RENEW_PAYMENT_DESCRIPTION =
  "Livecheck Sentinel watch renew - extend an active 30-day standard watcher. POST /v1/watch/renew with {id}. Requires X-Livecheck-Owner-Token plus payment. Same $2.50 USDC as POST /v1/watch create (2500000 atomic). One accept. Adds one prepaid term to expires_at and checks_remaining. Same watcher, not a second product. Stopped or expired cannot renew. owner_token is not returned again. Not /v1/watch/fast and not Confirm.";
export const OPENAPI_WATCH_RENEW_SUMMARY = "Renew an active 30-day watcher ($2.50)";
export const OPENAPI_WATCH_RENEW_DESCRIPTION =
  "POST {id} to extend an active standard watcher. Same $2.50 USDC (2500000 atomic) as POST /v1/watch — one accept, no dual price. id is the watcher id (wtc_ + ULID) in the JSON body so /.well-known/x402 can list a concrete URL (no {id} template). Requires X-Livecheck-Owner-Token in addition to payment (same as GET/DELETE /v1/watch/{id} and chain topup). Extends expires_at by 30 days from the later of now and the current expiry, and adds one term of checks_remaining (interval-based, max 2880/term). Resets the 24h expiring notice so it can fire again. Same watcher id, target, condition, callback, and chain balance. Does not recapture baseline or return owner_token. Stopped or expired → 409 not_renewable. Missing owner token after pay → 401. Wrong token → 403. Unknown id → 404. Unpaid → 402 with one $2.50 accept. Not Playwright / /v1/watch/fast. Not Confirm chain.";
