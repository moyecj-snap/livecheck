...
function mockPaymentMiddleware(): MiddlewareHandler {
  return async (c, next) => {
    const paidConfirm = c.req.method === "POST" && c.req.path === "/v1/confirm";
    const paidVerify = c.req.method === "POST" && c.req.path === "/v1/verify";
    if (!paidConfirm && !paidVerify) {
      return next();
    }
