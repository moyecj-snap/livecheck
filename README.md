...
The 402 `resource.url` is pinned to `https://livecheck.fly.dev/v1/watch`. Payment description is short ASCII (under 300 characters). CDP rejects `paymentPayload.resource.description` longer than 500 characters, and purl copies that field from the 402. Rich copy stays on `/health` and OpenAPI. `advertisePaymentRequired` does not change `amount` / `asset` / `payTo` / `network` / `scheme` / `extra` / `maxTimeoutSeconds`. `extra` is Verify's USDC domain `{name:"USD Coin", version:"2"}`. The 402 omits `extensions.bazaar` (same settle hotfix as Confirm `b7ab919`). OpenAPI still documents the watch schema. Renew and chain top-up keep their smaller bazaar extensions.
